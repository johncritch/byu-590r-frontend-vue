...
class UserService {
	...
	addDeviceToCollection(deviceId) {
		return axios
			.post(API_URL + `user/add_device_to_collection/${deviceId}`, null, {
				headers: authHeader()
			})
			.then((response) => response.data)
	}
	removeDeviceFromCollection(deviceId) {
		return axios
			.delete(API_URL + `user/remove_from_collection/${deviceId}`, {
				headers: authHeader()
			})
